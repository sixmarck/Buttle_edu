...
public class AdminDAO {
	...
	public ArrayList<AgentByResDTO> allReservationList () throws SQLException {
		...
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		ArrayList<AgentByResDTO> list = null;
		
		try {
			con = DBUtil.getConnection();
			pstmt = con.prepareStatement(info.getProperty("allReservationList"));
			rset = pstmt.executeQuery();
			list = new ArrayList<AgentByResDTO>();
			while (rset.next()) {
				list.add(new AgentByResDTO(rset.getInt(1), rset.getString(2), rset.getString(3), rset.getString(4), rset.getString(5),rset.getString(6), rset.getInt(7),rset.getInt(8)));
			}
		} finally {
			DBUtil.close(con, pstmt, rset);
		}
